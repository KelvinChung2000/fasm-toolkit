...
def test_blank_file_is_empty() -> None:
    assert parse_file(example("blank.fasm")).lines == ()


def test_blank_string_is_empty() -> None:
    assert parse_string("").lines == ()
    assert parse_string("\n\n\n").lines == ()


def test_comment_only_line_keeps_text_after_hash() -> None:
...
def test_comment_preserves_trailing_whitespace() -> None:
    assert parse_string("#   ").lines == (FasmLine(comment="   "),)


def test_implicit_one_feature() -> None:
...
def test_explicit_one_differs_from_implicit_one() -> None:
...
def test_single_bit_address() -> None:
...
def test_range_address_high_low_order() -> None:
...
def test_feature_annotation_and_comment_on_one_line() -> None:
