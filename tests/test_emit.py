...
def test_implicit_one_emits_without_equals():
    assert parse_string("feat").to_string() == "feat\n"


def test_explicit_one_emits_with_equals():
    assert parse_string("feat = 1").to_string() == "feat = 1\n"


def test_single_bit_address_emits_index_form():
    assert parse_string("a.INIT[17]").to_string() == "a.INIT[17]\n"


def test_range_emits_high_low():
    assert parse_string("a[63:32] = 32'hFF").to_string() == "a[63:32] = 32'hFF\n"


def test_width_is_derived_from_address_not_literal():
