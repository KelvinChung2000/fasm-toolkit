...
_ORIGINAL_PYTHON = (
    Path(__file__).resolve().parents[2] / "fasm" / ".venv" / "bin" / "python"
)
...
def test_canonical_output_matches_reference(name: str) -> None:
    path = example(name)
...
@pytest.mark.parametrize("name", EXAMPLES)
def test_default_output_matches_reference(name: str) -> None:
    path = example(name)
